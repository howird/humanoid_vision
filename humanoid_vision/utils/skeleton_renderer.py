...
class SkeletonRenderer:
    ...
    def __call__(
        ...
        gt_keypoints_3d[:, :, :-1] = (
            gt_keypoints_3d[:, :, :-1] - gt_keypoints_3d[:, [25 + 14], :-1] + pred_keypoints_3d[:, [25 + 14]]
        )
        ...
        if camera_translation is None:
            camera_translation = (
                torch.tensor([0.0, 0.0, 2 * self.cfg.EXTRA.FOCAL_LENGTH / (0.8 * self.cfg.MODEL.IMAGE_SIZE)])
                .unsqueeze(0)
                .repeat(batch_size, 1)
            )
        else:
        ...
        camera_center = (
            torch.tensor([self.cfg.MODEL.IMAGE_SIZE, self.cfg.MODEL.IMAGE_SIZE], dtype=torch.float).reshape(1, 2) / 2.0
        )
        ...
        pred_keypoints_3d_proj = (
            torch.cat([pred_keypoints_3d_proj, keypoints_to_render.reshape(batch_size, -1, 1)], dim=-1).cpu().numpy()
        )
        rows = []
        ...
        gt_keypoints_3d_proj_side = (
            torch.cat([gt_keypoints_3d_proj_side, gt_keypoints_3d_side[:, :, [-1]]], dim=-1).cpu().numpy()
        )
        pred_keypoints_3d_proj_side = (
            torch.cat([pred_keypoints_3d_proj_side, keypoints_to_render.reshape(batch_size, -1, 1)], dim=-1)
            .cpu()
            .numpy()
        )
        for i in range(batch_size):
            ...
            gt_keypoints_3d_proj_img = render_openpose(img, body_keypoints_3d_proj) / 255.0
            ...
            gt_keypoints_3d_proj_img_side = render_openpose(side_img, body_keypoints_3d_proj) / 255.0
            # pred 3D keypoints
            ...
            pred_keypoints_3d_proj_imgs.append(render_openpose(img, body_keypoints_3d_proj) / 255.0)
            ...
            pred_keypoints_3d_proj_imgs_side.append(render_openpose(side_img, body_keypoints_3d_proj) / 255.0)

...
Composes the pipeline stages (detection, feature extraction, association) and
the deep_sort tracker directly, with HMAR, HMR2, and the UV texture renderer
instantiated as three independent models.
"""
...
from humanoid_vision.configs.base import PhalpConfig, CACHE_DIR
from humanoid_vision.utils.video_io_manager import VideoIOManager
from humanoid_vision.utils.video import get_list_of_shots
...
def track_video_modular(
    ...
    Args:
        ...
        hmr2_model: HMR2 model for pose/shape
        hmar_model: HMAR model for appearance/UV processing
    ...
    list_of_shots = (
        get_list_of_shots(video_name, list_of_frames, cfg.video_io.output_dir)
        if cfg.detect_shots
        else []
    )
    ...
    for t, frame_path in progress_bar(
        ...
        log.debug(f"[{t + 1}/{len(list_of_frames)}] Processing {frame_name}")
        ...
        if len(bbox) == 0:
            log.debug(f"No valid detections in {frame_name}")
        ...
        hmar_out, appe_emb, pose_emb, loca_emb, joints, joints_2d, cam = (
            feature_extraction.extract_hmr_features(
                hmr2_model=hmr2_model,
                hmar_model=hmar_model,
...
def main():
    ...
    hmar_model.eval()
    hmar_model.to(cfg.device)
    log.info("  ✓ HMAR loaded")

    # HMR2 (pose/shape)
    download_models()
    hmr2_model = HMR2.load_from_checkpoint(
        CACHE_DIR
    ...
    hmr2_model.eval()
    hmr2_model.to(cfg.device)
    log.info("  ✓ HMR2 loaded")

    # UV Texture Renderer
    uv_renderer = UVTextureRenderer()
    uv_renderer.to(cfg.device)
    log.info("  ✓ UVTextureRenderer initialized")

    pose_predictor = setup_predictor(cfg, hmr2_model.smpl)
    pose_predictor.eval()
    ...
    if cfg.render.enable:
        ...
        visualizer = Visualizer(
            cfg.render, hmr2_model.smpl, cfg.EXTRA.FOCAL_LENGTH, cfg.SMPL.TEXTURE
        )
        visualizer.reset_render(cfg.render.res * cfg.render.up_scale)

        with VideoWriter(cfg.video_io, cfg.render.fps) as vwriter:
            for i, frame_key in progress_bar(
                enumerate(sorted(tracklets_data.keys())),
                description=f"Rendering : {video_name}",
                total=len(tracklets_data),
                disable=False,
            ):
